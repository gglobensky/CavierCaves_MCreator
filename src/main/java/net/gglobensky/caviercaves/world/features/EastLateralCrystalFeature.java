...
import net.gglobensky.caviercaves.procedures.CreateEastLateralCrystalProcedure;

import java.util.Set;
import java.util.List;

public class EastLateralCrystalFeature extends Feature<NoneFeatureConfiguration> {
	public static EastLateralCrystalFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new EastLateralCrystalFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("caviercaves:east_lateral_crystal", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("caviercaves:east_lateral_crystal", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public EastLateralCrystalFeature() {
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.NONE).setRotation(Rotation.NONE).setRandom(context.random()).addProcessor(BlockIgnoreProcessor.AIR).setIgnoreEntities(false),
						context.random(), 2)) {
					CreateEastLateralCrystalProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
