...
import net.gglobensky.caviercaves.procedures.CreateLargeAscendingCrystalProcedure;

import java.util.Set;
import java.util.List;

public class LargeAscendingCrystalFeature extends Feature<NoneFeatureConfiguration> {
	public static LargeAscendingCrystalFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new LargeAscendingCrystalFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("caviercaves:large_ascending_crystal", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("caviercaves:large_ascending_crystal", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public LargeAscendingCrystalFeature() {
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.NONE).setRotation(Rotation.NONE).setRandom(context.random()).addProcessor(BlockIgnoreProcessor.AIR).setIgnoreEntities(false),
						context.random(), 2)) {
					CreateLargeAscendingCrystalProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
