...
import net.gglobensky.caviercaves.procedures.CreateLargeDescendingCrystalProcedure;

import java.util.Set;
import java.util.List;

public class LargeDescendingCrystalFeature extends Feature<NoneFeatureConfiguration> {
	public static LargeDescendingCrystalFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new LargeDescendingCrystalFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("caviercaves:large_descending_crystal", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("caviercaves:large_descending_crystal", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public LargeDescendingCrystalFeature() {
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.NONE).setRotation(Rotation.NONE).setRandom(context.random()).addProcessor(BlockIgnoreProcessor.AIR).setIgnoreEntities(false),
						context.random(), 2)) {
					CreateLargeDescendingCrystalProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
