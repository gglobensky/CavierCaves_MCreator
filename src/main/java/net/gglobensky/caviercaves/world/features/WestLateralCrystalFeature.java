...
import net.gglobensky.caviercaves.procedures.CreateWestLateralCrystalProcedure;

import java.util.Set;
import java.util.List;

public class WestLateralCrystalFeature extends Feature<NoneFeatureConfiguration> {
	public static WestLateralCrystalFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new WestLateralCrystalFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("caviercaves:west_lateral_crystal", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("caviercaves:west_lateral_crystal", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public WestLateralCrystalFeature() {
		...
		if ((context.random().nextInt(1000000) + 1) <= 1000000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.NONE).setRotation(Rotation.NONE).setRandom(context.random()).addProcessor(BlockIgnoreProcessor.AIR).setIgnoreEntities(false),
						context.random(), 2)) {
					CreateWestLateralCrystalProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
